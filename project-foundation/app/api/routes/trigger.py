...
async def resume_run(
    ...
    try:
        ...
        # Reset failed stage in checkpoint so resume can continue
        ws = entity.workspace_path
        from pathlib import Path as _Path
        import json as _json

        # Determine failed_stage from checkpoint, entity, or contract files on disk
        failed_stage = entity.failed_stage or "unknown"
        cp_path = _Path(ws) / "contracts" / "checkpoint.json"
        if cp_path.exists():
            cp = _json.loads(cp_path.read_text(encoding="utf-8"))
            failed_stage = cp.get("failed_stage") or failed_stage
            ...
            # Infer failed_stage from which contract files are present on disk
            _contracts = _Path(ws) / "contracts"
            _stage_contracts = [
                ("code_generation", _contracts / "approved-test-plan.json"),
                ("human_review",    _contracts / "approved-test-plan.json"),
                ("test_design",     _contracts / "test-plan.json"),
                ("inventory",       _contracts / "inventory.json"),
                ("crawler",         _contracts / "crawl-package.json"),
            ]
            for _stage_name, _contract_file in _stage_contracts:
                if _contract_file.exists():
                    # The last present contract tells us which stage came before the failure
                    _codegen_meta = _Path(ws) / "artifacts" / "generated-tests" / "playwright" / "code-generation-metadata.json"
                    if _stage_name == "code_generation" and not _codegen_meta.exists():
                        failed_stage = "code_generation"
                    elif _stage_name == "human_review" and (_contracts / "approved-test-plan.json").exists() and not (_Path(ws) / "artifacts" / "generated-tests" / "playwright" / "code-generation-metadata.json").exists():
                        failed_stage = "code_generation"
                    else:
                        failed_stage = _stage_name
                    break
