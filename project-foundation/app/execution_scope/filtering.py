...
def filter_scenarios_by_scope(
    ...
    for scenario in scenarios:
        if resolver.module_allowed(_scenario_module(scenario)):
            kept_scenarios.append(scenario)

    kept_scenario_ids = {
        (s.get("metadata") or {}).get("id") for s in kept_scenarios if (s.get("metadata") or {}).get("id")
    }

    for mod in modules:
        if not resolver.module_allowed(str(mod.get("name") or "")):
            continue
        mod_scenarios = [
            s for s in (mod.get("scenarios") or [])
            if not (s.get("metadata") or {}).get("id") or (s.get("metadata") or {}).get("id") in kept_scenario_ids
        ]
