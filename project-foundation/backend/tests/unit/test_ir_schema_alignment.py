...
class TestPromptCompleteness:
    ...
    def test_prompt_includes_metadata_schema(self):
        ...
        assert "## IR Schema (authoritative)" in prompt
    ...
    def test_prompt_includes_complete_example(self):
        ...
        assert isinstance(example_data["metadata"]["generator"], str)
...
class TestIRGenerationAgentParsing:
    ...
    @pytest.mark.asyncio
    async def test_agent_does_not_fabricate_required_metadata(self):
        """Agent must not fabricate required field values (schema-safe policy).

        `metadata.generator` is required and has no schema default, so the
        schema-aware repairer must NOT invent a value. The response is rejected
        with validation feedback instead.
        """
        ...
        with pytest.raises(AgentExecutionError):
            await agent._parse_ir_response(incomplete_ir_json)
