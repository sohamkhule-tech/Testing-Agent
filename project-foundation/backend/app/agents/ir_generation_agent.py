...
from app.core.ir.ir_pre_validator import IRPreValidator
...
from app.core.ir.validation_feedback import ValidationFeedbackBuilder
...
class IRGenerationAgent(LoggerMixin):
    ...
    def __init__(self, llm_client: ILLMClient, max_refinement_attempts: int = 3) -> None:
        ...
        self.schema_repairer = SchemaAwareRepairer()
        self.feedback_builder = ValidationFeedbackBuilder()
    ...
    async def _generate_ir(
        ...
        await _emit(self.run_id, EventType.SENDING_LLM_REQUEST, {
            ...
            "max_tokens": self.llm_client.default_max_tokens,
            "temperature": 0.3,
            "label": f"Sending request to {selected_model}...",
        })
        ...
        await _emit(self.run_id, EventType.LLM_CALL_STARTED, {
            ...
            "label": f"Calling {selected_model} for IR Generation",
        })

        ir = await self._complete_and_parse_ir(prompt, prompt_tokens, model=selected_model)
    ...
    def _ir_system_prompt(self) -> str:
        """System prompt for IR generation and refinement calls."""
        return self.prompt_composer.instruction_builder.build_ir_system_prompt()

    @staticmethod
    def _structured_output_rejection(error: Exception) -> bool:
        """Determine whether an LLM error indicates structured output is unsupported."""
    ...
    def _completion_kwargs(self) -> tuple[dict[str, Any], str]:
        """Build kwargs for the LLM completion call plus the output mode name."""
        from app.config import get_settings
        settings = get_settings()
        if settings.llm.llm_structured_output_enabled:
            return {"response_format": {"type": "json_object"}}, "structured_json"
        return {}, "raw_json"
    ...
    async def _call_llm(
        ...
        """Call the LLM, preferring structured JSON output with a raw fallback.

        Structured output is requested when llm_structured_output_enabled is
        set. If the provider rejects it (e.g. unsupported response_format), the
        call is retried once in raw mode. Only the mode is logged, never the
        prompt, response, or system prompt content.
        """
        kwargs, mode = self._completion_kwargs()
        selected_model = model or self.llm_client.model
        self.logger.info("llm_output_mode_selected", mode=mode, model=selected_model)
        try:
            ...
            if kwargs and self._structured_output_rejection(structured_err):
                self.logger.warning("structured_output_fallback_raw")
                return await asyncio.wait_for(
                    self.llm_client.complete(
                        prompt=prompt,
                        max_tokens=max_tokens,
    ...
    async def _complete_and_parse_ir(self, prompt: str, prompt_tokens: int, model: str | None = None) -> CodeGenerationIR:
        """Call the LLM and parse its IR JSON output, retrying on malformed JSON.

        Small models occasionally emit invalid JSON on long IR generations. A
        single bad response must not hard-fail the run, so the call is retried a
        bounded number of times. The temperature stays stable so retries differ
        only through the accumulated validation feedback, which is appended to
        each retry's prompt. The raw response is never logged; only parse status
        is.
        """
        import time
        last_error = ""
        ...
        for attempt in range(1, self.MAX_JSON_RETRIES + 1):
            await _emit(self.run_id, EventType.WAITING_FOR_LLM_RESPONSE, {
                "label": f"Waiting for {selected_model} response...",
                "model": selected_model,
                "estimated_prompt_tokens": prompt_tokens,
                "status": "generating",
            })
            ...
            if previous_errors:
                attempt_prompt = self._build_retry_feedback(prompt, previous_errors)
            ...
            self.logger.info(
                ...
                estimated_tokens=prompt_tokens,
                max_tokens=self.llm_client.default_max_tokens,
                temperature=self.IR_TEMPERATURE,
                attempt=attempt,
            ...
            try:
                response = await self._call_llm(
                    ...
                    max_tokens=self.llm_client.default_max_tokens,
                    temperature=self.IR_TEMPERATURE,
                    system_prompt=self._ir_system_prompt(),
                    outer_timeout=outer_timeout,
                    model=selected_model,
                )
            ...
            try:
                ...
                last_error = str(e)
                if last_error not in previous_errors:
    ...
    async def _parse_ir_response(self, response: str) -> CodeGenerationIR:
        ...
        try:
            ...
            # Step 4: Schema-safe repair if needed
            if not is_valid:
                self.logger.warning(
                ...
                # Attempt schema-safe repair (fills missing optional keys only)
                ir_data = self.schema_repairer.repair(ir_data)

                if self.schema_repairer.repairs_made:
                    self.logger.info(
                        "ir_auto_repaired",
                        repairs=self.schema_repairer.repairs_made
                    )

                    await _emit(self.run_id, EventType.IR_AUTO_REPAIR_SUCCESS, {
                        "label": "IR auto-repaired successfully",
                        "repairs_count": len(self.schema_repairer.repairs_made),
                        "repairs": self.schema_repairer.repairs_made,
                    })
                ...
                if not is_valid:
                    ...
                    raise AgentExecutionError(
                        f"IR validation failed after repair. Errors:\n{error_report}"
                    )
                else:
            ...
            # Pydantic validation errors get structured, LLM-consumable feedback
            if isinstance(e, ValidationError):
                feedback = self.feedback_builder.from_pydantic(e)
                rendered = self.feedback_builder.render(feedback)
                self.logger.error("pydantic_validation_failed", feedback=rendered)
                raise AgentExecutionError(
    ...
    def _format_validation_error_report(self, errors: list[str]) -> str:
        ...
        report_lines.extend([
            ...
            "Fix the reported errors and regenerate the complete IR."
        ])

        return "\n".join(report_lines)

    def _build_retry_feedback(self, prompt: str, previous_errors: list[str]) -> str:
        """Build a retry prompt that includes the previous attempt's validation errors."""
        error_lines = "\n".join(f"- {error}" for error in previous_errors)
        return f"""{prompt}

## IMPORTANT: Previous attempt was rejected

Your previous IR output was rejected with the errors below. Fix EVERY reported
error and then regenerate the COMPLETE IR. Do not omit any part of the test
plan just because some fields were already correct.
...
{error_lines}"""
    ...
    async def _validate_and_refine_ir(
        ...
        Returns:
            Tuple of (final IR, validation result, refinement attempts)
        """
        ...
        while refinement_attempts <= self.max_refinement_attempts:
            ...
            if validation_result.is_valid:
                ...
                return ir, validation_result, refinement_attempts
            ...
            ir = await self._refine_ir(ir, validation_result, model=model)
