...
class OpenAIClient(ILLMClient, LoggerMixin):
    ...
    @with_retry(
        ...
        initial_wait=3.0,
        max_wait=30.0,
        exceptions=(LLMRateLimitError, LLMTimeoutError, ConnectionError, TimeoutError, asyncio.TimeoutError),
    )
    async def complete(
        self,
        prompt: str,
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if "rate" in error_msg.lower() or "429" in error_msg.lower() or type(e).__name__ == "RateLimitError":
    ...
    async def complete_structured(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            error_msg = str(e)
            self.logger.error("llm_structured_error", error=error_msg, error_type=type(e).__name__)
            if "rate" in error_msg.lower() or "429" in error_msg.lower() or type(e).__name__ == "RateLimitError":
                raise LLMRateLimitError(f"Rate limit exceeded: {error_msg}")
            elif "timeout" in error_msg.lower():
                raise LLMTimeoutError(f"Request timeout: {error_msg}")
            raise LLMProviderError(f"Structured completion failed: {error_msg}")
    ...
    async def stream_complete(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            self.logger.info("llm_stream_complete", model=selected_model)
