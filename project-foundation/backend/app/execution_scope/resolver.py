...
class ExecutionScopeResolver:
    ...
    def evaluate(self, url: str, *, title: str | None = None) -> ScopeDecision:
        ...
        # Initial entrypoint target_url or base_url or standard landing routes (/login, /dashboard, /home, /) are ALWAYS allowed
        path = _url_path(canonical)
        entry_paths = [_url_path(u) for u in (self.target_url, self.base_url) if u]
        if (entry_paths and path in entry_paths) or path in ("/", "/login", "/dashboard", "/home", "/index", "/main", "/app", "/portal"):
            return ScopeDecision(allowed=True, reason="Initial target entrypoint/landing route.")
...
def build_scope_grep(plan: Any = None, *, scope: dict[str, Any] | None = None) -> str | None:
    ...
    if resolver._restricted:
        for profile in resolver._included_profiles:
            for term in profile.grep_terms:
                ...
                if term_clean and re.match(r"^[a-zA-Z0-9_\-]+$", term_clean) and term_clean not in terms:
    ...
    for coverage in resolver.coverage_preferences:
        ...
        if low in ("all", "full", "comprehensive", ""):
            continue
        sub_items = [s.strip() for s in re.split(r"[,;]+", low) if s.strip()]
        for item in sub_items:
            if item in _COVERAGE_GREP:
                for term in _COVERAGE_GREP[item]:
                    term_clean = term.strip()
                    if term_clean and re.match(r"^[a-zA-Z0-9_\-]+$", term_clean) and term_clean not in terms:
                        terms.append(term_clean)
            else:
                ...
                for word in words:
                    w = word.lower()
                    if w in ("scenarios", "testing", "handling", "tests", "test", "cases", "page", "module"):
                        continue
                    if w in _COVERAGE_GREP:
                        for term in _COVERAGE_GREP[w]:
                            term_clean = term.strip()
                            if term_clean and re.match(r"^[a-zA-Z0-9_\-]+$", term_clean) and term_clean not in terms:
                                terms.append(term_clean)
                    elif len(w) > 2 and w not in terms:
                        terms.append(w)
