...
function FileItem({ file }: { file: GeneratedFile }) {
  ...
  return (
    <div className="flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-zinc-800/60 transition-colors group animate-fade-in">
      <FileCode2 className={cn('h-3.5 w-3.5 shrink-0', extColors[ext] ?? 'text-zinc-400')} />
      <span className="text-xs font-mono text-zinc-300 flex-1 truncate">{file.path || file.name}</span>
      <span className="text-[10px] text-zinc-600 shrink-0 group-hover:text-zinc-400 transition-colors">
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs">
          <span className="text-zinc-400 font-medium">Generation Progress</span>
      ...
      {/* File list */}
      {files.length > 0 && (
        <div className="space-y-1 border border-zinc-800 rounded-xl overflow-hidden">
          <div className="px-3 py-2 border-b border-zinc-800 bg-zinc-900">
            <span className="text-[10px] text-zinc-500 uppercase tracking-wider font-semibold">Generated Files</span>
          </div>
...
function TestRow({ result }: { result: TestResult }) {
  return (
    <div className={cn(
      ...
      result.status === 'skipped' && 'border-zinc-700/50 bg-zinc-900/30',
    )}>
      ...
      {result.status === 'skipped' && <SkipForward  className="h-4 w-4 text-zinc-500 shrink-0" />}

      <span className="text-xs text-zinc-200 flex-1 truncate font-medium">{result.name}</span>
  ...
  if (exStage?.status === 'completed' && results.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-amber-500 gap-3">
        ...
        <p className="text-xs text-zinc-500">Playwright ran but results.json was not generated. Check the generated tests directory for details.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="col-span-1 flex flex-col items-center justify-center p-4 rounded-xl bg-zinc-900 border border-zinc-800">
          ...
          <p className="text-[10px] text-zinc-500 mt-1">Pass Rate</p>
        </div>

        <div className="col-span-3 grid grid-rows-2 grid-cols-2 gap-2">
          {[
            { label: 'Total',   value: stats.total,   color: 'text-zinc-300',   icon: TrendingUp },
            ...
            { label: 'Skipped', value: stats.skipped, color: 'text-zinc-400',   icon: SkipForward },
          ].map((s) => (
            <div key={s.label} className="flex items-center gap-2 p-2.5 rounded-lg bg-zinc-900 border border-zinc-800">
