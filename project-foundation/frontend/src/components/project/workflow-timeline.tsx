  ...
  if (!latestRun) {
    return (
      <Card className="border-border bg-card">
        ...
        <CardContent>
          <p className="text-xs text-muted-foreground text-center py-4">No activity yet. Start a run to track workflow timeline.</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <CardContent className="space-y-0">
        {events.map((ev, i) => (
          <div key={i} className="relative flex gap-3 pb-4 last:pb-0">
            {i < events.length - 1 && <div className="absolute left-[11px] top-5 h-full w-px bg-border" />}
            <div className={cn(
