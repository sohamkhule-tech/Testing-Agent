    ...
    <Card className="border-zinc-800 bg-zinc-950/60">
      <CardHeader className="pb-3">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-base text-zinc-200">AI Workflow Pipeline</CardTitle>
          {latestRunStatus && (
            ...
            return (
              <div key={stage.phase} className="flex items-center min-w-0">
                <div className="flex flex-col items-center gap-1.5 min-w-0">
                  ...
                  <span className={cn("text-[11px] font-medium whitespace-nowrap", (isPast || isCurrent) && "text-zinc-200", isPending && "text-zinc-500")}>{stage.label}</span>
                </div>
                {i < STAGES.length - 1 && (
                  <div className={cn("h-px w-8 sm:w-12 mx-1 mt-4", isPast ? "bg-emerald-500/60" : isCurrent ? "bg-blue-500/60" : "bg-zinc-800")} />
                )}
              </div>
            );
          })}
